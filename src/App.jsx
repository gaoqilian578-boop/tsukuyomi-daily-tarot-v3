...
const STORAGE = {
  profiles: "tsukuyomi-daily-tarot-v3-profiles",
  currentUserId: "tsukuyomi-daily-tarot-v3-current-user",
  draws: "tsukuyomi-daily-tarot-v3-draws",
  streaks: "tsukuyomi-daily-tarot-v3-streaks"
};
...
function createId(prefix) { return crypto?.randomUUID ? crypto.randomUUID() : `${prefix}-${Date.now()}-${Math.random().toString(16).slice(2)}`; }
function localDate() { const d = new Date(); return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`; }
function yesterdayOf(dateText) { const d = new Date(`${dateText}T00:00:00`); d.setDate(d.getDate() - 1); return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`; }
function makeOptions() { return [...tarotCards].sort(() => Math.random() - 0.5).slice(0, 3).map((card) => ({ card, position: Math.random() > 0.5 ? "正位置" : "逆位置" })); }
function resolveCard(draw) { return tarotCards.find((card) => card.id === draw?.cardId || card.nameJa === draw?.cardName) || tarotCards[0]; }
...
async function signUp(profile, password) {
  if (isSupabaseConfigured) {
    ...
    const savedProfile = { ...profile, userId: data.user?.id, createdAt: new Date().toISOString() };
    await supabase.from("profiles").upsert(savedProfile);
    return savedProfile;
  }
  const profiles = getStored(STORAGE.profiles, []);
  if (profiles.some((item) => item.email === profile.email)) throw new Error("このメールアドレスはすでに登録されています。ログインしてください。");
  const savedProfile = { ...profile, userId: createId("user"), createdAt: new Date().toISOString() };
  setStored(STORAGE.profiles, [...profiles, savedProfile]);
  setStored(STORAGE.currentUserId, savedProfile.userId);
  return savedProfile;
}
async function signIn(email, password) {
  ...
  const profile = getStored(STORAGE.profiles, []).find((item) => item.email === email);
  if (!profile || !password) throw new Error("ログイン情報を確認してください。");
  setStored(STORAGE.currentUserId, profile.userId);
  return profile;
}
async function fetchDraws(userId) {
  ...
  return getStored(STORAGE.draws, []).filter((draw) => draw.userId === userId).sort((a, b) => b.date.localeCompare(a.date));
}
async function saveDraw(draw) {
  ...
  setStored(STORAGE.draws, [...getStored(STORAGE.draws, []), draw]);
}
async function getStreak(userId) {
  ...
  return getStored(STORAGE.streaks, []).find((item) => item.userId === userId) || empty;
}
async function saveStreak(streak) {
  if (isSupabaseConfigured) { await supabase.from("streaks").upsert(streak); return; }
  setStored(STORAGE.streaks, [...getStored(STORAGE.streaks, []).filter((item) => item.userId !== streak.userId), streak]);
}
...
export default function App() {
  ...
  useEffect(() => {
    async function boot() {
      if (!isSupabaseConfigured) {
        const userId = getStored(STORAGE.currentUserId, null);
        const storedProfile = getStored(STORAGE.profiles, []).find((item) => item.userId === userId);
  ...
  function logout() {
    if (!isSupabaseConfigured) setStored(STORAGE.currentUserId, null);
    setProfile(null); setDraws([]); setStreak(null); setPage("lp");
  }
  const todayDraw = useMemo(() => draws.find((draw) => draw.date === localDate()), [draws]);
  if (loading) return <Shell><p className="loading">月を読み込んでいます...</p></Shell>;

  return (
    <Shell>
      <TopNav profile={profile} page={page} setPage={setPage} logout={logout} />
      {!profile && page === "lp" && <Landing onRegister={() => setPage("register")} onLogin={() => setPage("login")} />}
      {!profile && page === "register" && <RegisterPage onSubmit={refresh} onLogin={() => setPage("login")} />}
      {!profile && page === "login" && <LoginPage onSubmit={refresh} onRegister={() => setPage("register")} />}
      {profile && page === "tarot" && <TarotPage profile={profile} todayDraw={todayDraw} streak={streak} onSaved={refresh} />}
      {profile && page === "history" && <HistoryPage draws={draws} />}
    </Shell>
  );
}

function Shell({ children }) { return <main className="app"><div className="moon-glow" /><div className="stars-bg" />{children}</main>; }
function TopNav({ profile, page, setPage, logout }) {
  return <nav className="top-nav"><button className="brand" onClick={() => setPage(profile ? "tarot" : "lp")}>月読</button><div>{profile ? <><button className={page === "tarot" ? "active" : ""} onClick={() => setPage("tarot")}>今日</button><button className={page === "history" ? "active" : ""} onClick={() => setPage("history")}>履歴</button><button onClick={logout}>ログアウト</button></> : <><button className={page === "login" ? "active" : ""} onClick={() => setPage("login")}>ログイン</button><button className={page === "register" ? "active" : ""} onClick={() => setPage("register")}>登録</button></>}</div></nav>;
}
function Landing({ onRegister, onLogin }) {
  return <section className="hero"><p className="eyebrow">月読 ─ tsukuyomi ─</p><h1>月読｜今日のタロット</h1><p className="lead">1日1回だけ、今の状況と言葉をカードに預ける。恋愛、不安、復縁、仕事、人間関係を静かに整理する夜のタロットです。</p><div className="hero-note">自由記入の状況文、星座、今日のカードを組み合わせて、今の流れと次の一手をやさしく言語化します。</div><div className="button-row"><button className="primary-button" onClick={onRegister}>今日の月を開く</button><button className="ghost-button" onClick={onLogin}>ログイン</button></div>{!isSupabaseConfigured && <p className="dev-note">現在はSupabase未接続のため、localStorageで仮保存しています。</p>}</section>;
}
function RegisterPage({ onSubmit, onLogin }) {
  ...
  async function submit(event) {
    ...
    try {
      const required = ["nickname", "email", "password", "birthday", "gender", "zodiacSign", "situationText"];
      if (required.some((field) => !form[field])) throw new Error("必須項目を入力してください。");
      const { password, ...profileData } = form;
      onSubmit(await signUp(profileData, password));
...
function TarotPage({ profile, todayDraw, streak, onSaved }) {
  ...
  function startShuffle() { if (draw || busy) return; setError(""); setSelected(null); setOptions([]); setDrawState("shuffling"); window.setTimeout(() => { setOptions(makeOptions()); setDrawState("choosing"); }, 2800); }
  async function chooseCard(option) {
    if (draw || busy || selected) return; setBusy(true); setSelected(option); setDrawState("revealing"); setError("");
    try {
      const { card, position } = option; const reading = generateReading(profile, card, position, profile.situationText, profile.currentActionText); const date = localDate();
      const savedDraw = { id: createId("draw"), userId: profile.userId, date, cardId: card.id, cardName: card.nameJa, position, situationText: profile.situationText, currentActionText: profile.currentActionText, ...reading, createdAt: new Date().toISOString() };
      await new Promise((resolve) => window.setTimeout(resolve, 1100)); await saveDraw(savedDraw);
      const current = await getStreak(profile.userId); const nextStreak = current.lastDrawDate === yesterdayOf(date) ? current.currentStreak + 1 : current.lastDrawDate === date ? current.currentStreak : 1;
      await saveStreak({ userId: profile.userId, currentStreak: nextStreak, lastDrawDate: date, totalDraws: current.totalDraws + 1, threeDayOfferShown: current.threeDayOfferShown || nextStreak >= 3, offerClicked: current.offerClicked || false });
      setDraw(savedDraw); setDrawState("revealed"); await onSaved(profile);
    } catch (err) { setError(err.message); setDrawState("choosing"); setSelected(null); } finally { setBusy(false); }
  }
...
function ResultPage({ draw, streak, userId }) {
  const card = resolveCard(draw); const showOffer = (streak?.currentStreak || 0) >= 3;
