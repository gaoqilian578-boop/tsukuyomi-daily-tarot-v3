...
// Production 月読 tarot app. This file intentionally replaces the old deploy-check placeholder.
...
function createOptions() {
  return [...tarotCards]
    ...
    .slice(0, 3)
    .map((card) => ({ card, position: Math.random() > 0.5 ? "正位置" : "逆位置" }));
}
function resolveCard(draw) {
  return tarotCards.find((card) => card.id === draw?.cardId || card.nameJa === draw?.cardName) || tarotCards[0];
}
...
async function signUp(profile, password) {
  if (isSupabaseConfigured) {
    const { data, error } = await supabase.auth.signUp({
      email: profile.email,
      password,
      options: { data: { nickname: profile.nickname } }
    });
  ...
  if (profiles.some((item) => item.email === profile.email)) {
    throw new Error("このメールアドレスはすでに登録されています。ログインしてください。");
  }
  const saved = { ...profile, userId: createId("user"), createdAt: new Date().toISOString() };
  writeStore(STORAGE.profiles, [...profiles, saved]);
  writeStore(STORAGE.currentUserId, saved.userId);
  return saved;
}
async function signIn(email, password) {
  ...
  const profile = readStore(STORAGE.profiles, []).find((item) => item.email === email);
  if (!profile || !password) throw new Error("ログイン情報を確認してください。");
  writeStore(STORAGE.currentUserId, profile.userId);
  return profile;
}
async function fetchDraws(userId) {
  ...
  return readStore(STORAGE.draws, []).filter((draw) => draw.userId === userId).sort((a, b) => b.date.localeCompare(a.date));
}
async function saveDraw(draw) {
  ...
  writeStore(STORAGE.draws, [...readStore(STORAGE.draws, []), draw]);
}
async function getStreak(userId) {
  const fallback = { userId, currentStreak: 0, lastDrawDate: "", totalDraws: 0, threeDayOfferShown: false, offerClicked: false };
  if (isSupabaseConfigured) {
    const { data } = await supabase.from("streaks").select("*").eq("userId", userId).single();
    return data || fallback;
  }
  return readStore(STORAGE.streaks, []).find((item) => item.userId === userId) || fallback;
}
async function saveStreak(streak) {
  ...
  writeStore(STORAGE.streaks, [...readStore(STORAGE.streaks, []).filter((item) => item.userId !== streak.userId), streak]);
}
async function markOfferClicked(userId) {
  const streak = await getStreak(userId);
  await saveStreak({ ...streak, offerClicked: true });
}
...
export default function App() {
  ...
  useEffect(() => {
    async function boot() {
      if (!isSupabaseConfigured) {
        const userId = readStore(STORAGE.currentUserId, null);
        const stored = readStore(STORAGE.profiles, []).find((item) => item.userId === userId);
  ...
  function logout() {
    writeStore(STORAGE.currentUserId, null);
    setProfile(null);
    setDraws([]);
    setStreak(null);
    setPage("lp");
  ...
  const todayDraw = useMemo(() => draws.find((draw) => draw.date === todayText()), [draws]);
...
function Shell({ children }) {
  return <main className="app"><div className="moon-glow" /><div className="stars-bg" />{children}</main>;
}
function TopNav({ profile, page, setPage, logout }) {
  return (
    <nav className="top-nav">
      <button className="brand" onClick={() => setPage(profile ? "tarot" : "lp")}>月読</button>
      <div>{profile ? <><button className={page === "tarot" ? "active" : ""} onClick={() => setPage("tarot")}>今日</button><button className={page === "history" ? "active" : ""} onClick={() => setPage("history")}>履歴</button><button onClick={logout}>ログアウト</button></> : <><button className={page === "login" ? "active" : ""} onClick={() => setPage("login")}>ログイン</button><button className={page === "register" ? "active" : ""} onClick={() => setPage("register")}>登録</button></>}</div>
    </nav>
  );
}
...
function TarotPage({ profile, todayDraw, streak, onSaved }) {
  ...
  function startShuffle() { if (draw || busy) return; setError(""); setSelected(null); setOptions([]); setDrawState("shuffling"); window.setTimeout(() => { setOptions(createOptions()); setDrawState("choosing"); }, 2800); }
  async function chooseCard(option) {
    ...
    try {
      const { card, position } = option; const reading = generateReading(profile, card, position, profile.situationText, profile.currentActionText); const date = todayText();
